...
const InSeasonPage = () => {
  ...
  // Initialize searchTerm from router state `q` or query param `q` when landing on /season
  ...
  // Set fixed categories: fruits and Seasonal fruits
  useEffect(() => {
    if (categories.length > 0) {
      const fruitsCategory = categories.find(cat => cat.name.toLowerCase() === 'fruits');
      if (fruitsCategory) {
        const seasonalSub = fruitsCategory.subCates?.find(sub => sub.name.toLowerCase() === 'seasonal fruits');
        if (seasonalSub) {
          filterByCategory(fruitsCategory.id, seasonalSub.id);
        } else {
          filterByCategory(fruitsCategory.id);
        }
      }
    }
  ...
  // Submit search: navigate to /season with router state (do not use query string)
  const handleSearchSubmit = (e) => {
    ...
    try {
      ...
      navigate('/season', { state: baseState });
    } catch (err) {
      // fallback: do nothing
    }
  };
