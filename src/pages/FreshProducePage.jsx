...
const FreshProducePage = () => {
  ...
  // Initialize searchTerm from router state `q` or query param `q` when landing on /produce
  ...
  // Set fixed category: Fresh Produce
  useEffect(() => {
    if (categories.length > 0) {
      const freshProduceCategory = categories.find(cat => cat.name.toLowerCase() === 'fresh produce');
      if (freshProduceCategory) {
        filterByCategory(freshProduceCategory.id);
      }
    }
  ...
  // Submit search: navigate to /produce with router state (do not use query string)
  const handleSearchSubmit = (e) => {
    ...
    try {
      ...
      navigate('/produce', { state: baseState });
    } catch (err) {
      // fallback: do nothing
    }
  };
